...
export default function DataDisplay({ data, onProjectSelect, onClientSelect }) {
        ...
        return ( <
            ...
            div className = "all-projects-view" > {
                allProjects.map((project) => ( <
                    ...
                    /div> <
                    span style = {
                        { fontSize: '0.85rem', color: '#666', whiteSpace: 'nowrap', marginLeft: '10px' } } > { partnerMap[project.id] } <
                    /span> <
                    /div>
                        ...
                        project.clients && project.clients.length > 0 && ( <
                            ...
                            div className = "clients-grid" > {
                                project.clients.map((client) => ( <
                                    ...
                                    <
                                    h6 > { client.name } < /h6> <
                                    span className = { `status-badge ${client.status}` } > { client.status } <
                                    /span> {
                                        client.statistics && ( <
    ...
    if (data.filterType === 'project' || data.filterType === 'client') {
        if (data.data && data.data[0]) {
            return ( <
                ProjectDetailView project = { data.data[0] }
    ...
    return ( <
        div className = "data-display" > { /* Partner Info */ } <
